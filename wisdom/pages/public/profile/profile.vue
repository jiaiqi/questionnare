...
<script>
	export default {
      ...
      toRepair(){
        ...
        uni.navigateTo({
        	url:'/pages/public/list/list?serviceName=srvzhxq_repairs_select&pageType=list&viewTemp=' + decodeURIComponent(JSON.stringify(viewTemp)) + '&cond=' + decodeURIComponent(JSON.stringify(cond)) 
        })
		}else{
			uni.showToast({
				title:'未进行住户登记',
			...
			toMyCar(){
				...
				uni.navigateTo({
					url:'/pages/public/list/list?serviceName=srvzhxq_clgl_select&pageType=list&type=skip&viewTemp=' + decodeURIComponent(JSON.stringify(viewTemp)) + '&cond=' + decodeURIComponent(JSON.stringify(cond)) + '&tempWord=' + decodeURIComponent(JSON.stringify(viewLeftTemp))
				})
				}else{
					uni.showToast({
						title:'未进行住户登记',
			...
			async toVisitor(){
				...
				if(res.data.data.length>0){
					...
					uni.navigateTo({
						url:'/pages/public/list/list?serviceName=srvzhxq_guest_mgmt_select&pageType=list&type=skip&viewTemp=' + decodeURIComponent(JSON.stringify(viewTemp)) + '&cond=' + decodeURIComponent(JSON.stringify(cond)) + '&tempWord=' + decodeURIComponent(JSON.stringify(viewLeftTemp))
					})
				}
				}else{
					uni.showToast({
