<template>
	<view class="authorization">
		<view class="container" v-if="isBindUser || client_env === 'web' || client_env === 'app' || client_env === 'wxmp'">
		...
		saveWxUser() {
			...
			wx.getUserInfo({
				success: function(res) {
					...
					that.getWxUserInfo(res.userInfo);
					...
					success(res) {
						...
						uni.getUserInfo({
							...
							success: function(infoRes) {
								...
								that.getWxUserInfo(infoRes.userInfo);
								uni.showModal({
									title: '提示',
									content: '授权成功，即将返回上一页面',
									showCancel: false,
				...
				that.$http.post(url, req).then(response => {
					if (response.data.resultCode === 'SUCCESS' && response.data.response[0].response) {
						...
						if (resData.login_user_info && resData.login_user_info.user_no) {
							...
							// that.$store.commit('setLoginUserInfo', resData.login_user_info);
