  ...
  <view>
      ...
      >
        ...
        <radio-group @change="radioChange" v-else-if="fieldData.type === 'radioFk'" :class="!valid.valid ? 'valid_error' : ''">
          <radio
            ...
            class="blue radio"
            :disabled="fieldData.disabled ? fieldData.disabled : false"
            :checked="!!fieldData.value && (itema.value === fieldData.value || itema.value === fieldData.defaultValue)"
            :value="itema.value"
          >
        ...
        <checkbox-group name="checkbox-group" class="checkbox-group" @change="radioChange" v-else-if="fieldData.type === 'checkbox'" :class="!valid.valid ? 'valid_error' : ''">
          <label v-for="(item, index) in fieldData.options" :key="index" class="checkbox">
            <checkbox
              ...
              :value="item"
              :disabled="fieldData.disabled ? fieldData.disabled : false"
              :checked="fieldData.value && iArray(fieldData.value) ? fieldData.value.includes(item) : false"
            />
        ...
        <checkbox-group name="checkbox-group" class="checkbox-group" @change="radioChange" v-else-if="fieldData.type === 'checkboxFk'" :class="!valid.valid ? 'valid_error' : ''">
          <label v-for="(item, index) in fieldData.options" :key="index" class="checkbox">
            <checkbox
              ...
              :value="item.key"
              :disabled="fieldData.disabled ? fieldData.disabled : false"
              :checked="fieldData.value && iArray(fieldData.value) ? fieldData.value.includes(item.key) : false"
            />
    ...
    <view class="cu-modal bottom-modal" :class="{ show: showTreeSelector }">
      <view class="cu-dialog tree-selector">
        <bxTreeSelector
          :srvInfo="Array.isArray(fieldData.option_list_v2) ? null : fieldData.option_list_v2"
          :treeData="treeSelectorData"
          :childNodeCol="'_childNode'"
          :disColName="fieldData.option_list_v2 ? fieldData.option_list_v2['key_disp_col'] : ''"
          :nodeKey="fieldData.option_list_v2 && fieldData.option_list_v2['refed_col'] ? fieldData.option_list_v2['refed_col'] : 'no'"
...
export default {
  ...
  methods: {
    ...
    async getShareRoomNum() {
      ...
      console.log('业主信息----', res.data.data);
    ...
    async getTreeSelectorData(cond) {
      let self = this;
      if (this.service && (this.service == 'srvzhxq_guest_mgmt_yezhu_add' || this.service == 'srvzhxq_guest_mgmt_yezhu_update')) {
        this.getShareRoomNum();
      }
      let req = {
      ...
      // #endif
      if (this.fieldData.option_list_v2.conditions && Array.isArray(this.fieldData.option_list_v2.conditions) && this.fieldData.option_list_v2.conditions.length > 0) {
        ...
        // req.condition = condition&&condition[0]?condition:[];
      }
      if (cond) {
        req.condition = cond;
      }
      ...
      if (res.data.state === 'SUCCESS' && res.data.data.length > 0) {
        ...
        self.treeSelectorData.forEach(item => {
          if (item[this.fieldData.option_list_v2.refed_col] === this.fieldData.value) {
