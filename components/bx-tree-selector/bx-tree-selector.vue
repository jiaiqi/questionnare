  ...
  <view>
    <view class="padding-sm flex flex-wrap" @click="onShowModal">
      <view class="padding-sm flex flex-wrap">
        <view v-if="!isTree" class="" v-for="(item, index) in selected" :key="index">
          <view class="cu-tag cf line-green padding-xs">
            {{ item[options.key_disp_col] }}
            <text class="fr lg text-green cuIcon-check"></text>
          </view>
        </view>

        <view v-if="isTree" class="" v-for="(item, index) in selectedPathNodeDatas" :key="index">
          <view class="cu-tag cf  padding-xs" :class="index === selectedPathNodeDatas.length - 1 ? 'line-green' : 'line-blue'">
            {{ item[options.key_disp_col] }}
            <text v-if="index === selectedPathNodeDatas.length - 1" class="fr lg text-green cuIcon-check"></text>
          </view>
          <text class="lg text-gray cuIcon-right"></text>
        </view>
        <view class="" v-if="isTree"><text class="cu-tag line-gray padding-xs">更多</text></view>
      </view>
    </view>
          ...
          <view v-if="isTree">
            <view class="padding-sm flex flex-wrap" :class="layoutClass">
              <view class="cf" v-for="(item, index) in selectedPathNodeDatas" @click.stop="onSelectedPath(item, index)" :key="index">
                <view class="cu-tag cf  padding-xs margin-xs" :class="index === selectedPathNodeDatas.length - 1 ? 'line-green' : 'line-blue'">
                  {{ item[options.key_disp_col] }}
                  <text v-if="index === selectedPathNodeDatas.length - 1" class="fr lg text-green cuIcon-check"></text>
                </view>
            ...
            <view class="padding-sm flex flex-wrap solid bg-white">
              <view class="padding-xs" v-for="(item, index) in treeActiveNodes" :key="index" @click="onSelected(item, index)">
                <view class="cf cu-tag " :class="isInSelected(item) ? 'line-green' : 'line-gray'">
                  {{ item[options.key_disp_col] }}
  ...
  methods: {
    ...
    loadTreeData(e) {
      ...
      if (this.dataLoadType === 'nodes') {
        ...
        if (this.options.conditions.length > 0 && this.options.conditions[0].value && this.options.conditions[0].value.indexOf("'") > -1) {
        ...
        } else if (this.options.conditions.length > 0 && this.options.conditions[0].value && this.options.conditions[0].value.indexOf('data') > -1) {
        ...
        this.onRequest('select', this.options.serviceName, req, app).then(res => {
          // debugger
          self.treeDatas = res.data.data;
          if (self.isTree) {
            self.bxtreeDatas = self.treeReform(self.treeDatas, self.treeForeignKey.parentNoColName, self.treeForeignKey.colKeyName);
            // 封装并保存 tree 数据
          }
          self.onInitialSelected(self.defaultValue); // 初始化值显示
        });
      }
    }
  },
