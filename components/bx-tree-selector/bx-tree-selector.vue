      ...
      <view class="padding-sm flex flex-wrap" @click="onShowModal">
        <view class="flex flex-wrap">
          ...
          <view v-if="isTree && selectedPathNodeDatas.length > 0" class="" v-for="(item, index) in selectedPathNodeDatas" :key="index">
            <view class="cu-tag text-lg round cf   padding-sm" :class="index === selectedPathNodeDatas.length - 1 ? 'line-green' : 'line-blue'">
              {{ item[options.key_disp_col] }}
              <text v-if="index === selectedPathNodeDatas.length - 1" class="fr lg text-green cuIcon-check"></text>
            </view>
            <text class="lg text-gray cuIcon-right"></text>
          </view>
          <view class="" v-if="isTree && selected.length === 0"><!-- <text class="cu-tag line-gray  padding-sm">更多</text> --></view>
          <view class="" v-if="isTree && selectedPathNodeDatas.length === 0"><text class="cu-tag text-lg round line-gray  padding-sm">全部</text></view>
          <view class="" v-if="!isTree && selected.length === 0"><text class="cu-tag text-lg line-gray  padding-sm">未选择</text></view>
        </view>
      </view>
    </scroll-view>
            ...
            <scroll-view scroll-x class="bg-white padding response cu-steps steps-bottom" scroll-with-animation>
              <view class="padding-sm flex flex-wrap" :class="layoutClass">
                <view class="cf" v-for="(item, index) in selectedPathNodeDatas" @click.stop="onSelectedPath(item, index)" :key="index">
                  <view class="cu-tag text-lg round cf  padding-sm margin-xs" :class="index === selectedPathNodeDatas.length - 1 ? 'line-green' : 'line-blue'">
                    {{ item[options.key_disp_col] }}
                    <text v-if="index === selectedPathNodeDatas.length - 1" class="fr lg text-green cuIcon-check"></text>
                  </view>
                ...
                <view class="" v-if="selectedPathNodeDatas.length === 0"><text class="cu-tag text-lg round line-gray  padding-sm">请选择</text></view>
              </view>
            </scroll-view>
    ...
    loadTreeData(e) {
      ...
      if (this.dataLoadType === 'nodes') {
        ...
        if (this.options.conditions.length > 0 &&this.options.conditions[0].value &&this.options.conditions[0].value.indexOf("'") > -1) {
          ...
          if (this.options.conditions[0].value && this.options.conditions[0].value.indexOf('data') > -1) {
